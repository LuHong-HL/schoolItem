    ...
    <div class="mavon-editor">
      <div class="mavon-header">
        ...
        <a-date-picker showTime placeholder="开始时间" style="margin-left: 12px;margin-right:20px;width: 400px;"
          id="startTime" format="YYYY-MM-DD HH:mm"  v-model="startTime"/>
        <label for="endTime">结束时间：</label>
        <a-date-picker showTime placeholder="结束时间"  style="width: 400px;" id="endTime" 
          format="YYYY-MM-DD HH:mm"  v-model="endTime"/>
        </p>-->
        <p style="margin-top:20px" v-for="(item,index) in people" :key="item">
          ...
          <a-button type="danger" v-if="item !== people ">删除</a-button>

          <a-button type="primary" v-if="item == people " ghost>添加</a-button>
        </p>
        <div style="margin-top: 20px;">
          <label for="ativity" style="float: left;">活动海报：</label>
          <a-upload listType="picture-card" :fileList="fileList" :multiple="true" id="ativity">
      ...
      <!-- 按钮 -->
      <div class="mavon-btn">
        <a-button type="primary" ghost>保存至草稿箱</a-button>
        <a-button type="primary" ghost style="margin-left: 10px;">草稿箱</a-button>
        <a-button type="primary" ghost style="margin-left: 10px;" v-if="status == 2">发布</a-button>
        <a-button type="primary" ghost style="margin-left: 10px;" v-if="status == 1">修改</a-button>
      </div>
      ...
      >
        <ul>
          <li v-for="item in draftList" :key="item.articleId">
      ...
      <!-- <mavon-editor v-model="description" :ishljs="true" ref=md ></mavon-editor> -->
    </div>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    // getfullName(id) {
    //   if (id == 1) {
    //     this.axios.get("/organization/list/" + id).then(result => {
    //       console.log("optionsData:", result);
    //       let item;
    ...
    //   }else{
    //     this.axios.get("/organization/list/" + id).then(result => {
    //       console.log("optionsDatason:", result);
    //       let item;
    //       if (result.data.code == 0) {
    //         let data = result.data.data;
    //         for (let i = 0; i < data.length; i++) {
    ...
    loadData(selectedOptions) {
      ...
      console.log("targetedOption", targetOption.id);
      this.getfullName(targetOption.id);
